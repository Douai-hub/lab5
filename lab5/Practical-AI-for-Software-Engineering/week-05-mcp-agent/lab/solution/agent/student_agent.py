...
class MCPClient:
    ...
    async def connect_all(self):
        ...
        Challenge 2 solved: one unavailable server no longer crashes the app.
        We try each server, record any that are down in self.unavailable, and
        continue with whatever connected successfully.
        """
        for name, url in self._targets():
            try:
                ...
                print(f"  [warning] '{name}' MCP server unavailable at {url}: "
                      f"{type(exc).__name__}")
